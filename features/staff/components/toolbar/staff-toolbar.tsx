...
export function StaffToolbar({
    ...
    role, onRoleChange, status, onStatusChange
}: StaffToolbarProps) {
    return (
        <Card className="grid grid-cols-1 md:grid-cols-3 items-center justify-between gap-2 px-6 py-3">
            ...
            <StaffFilters
                ...
                department={department} onDepartmentChange={onDepartmentChange}
                role={role} onRoleChange={onRoleChange}
            />
            <StaffToolbarActions onExport={() => { }} onNewStaff={() => { }} canCreateStaff={true} />
        </Card>
    )
}